...
-- Simplified network statistics per ship
...
-- Simplified performance impacts from gossip
CREATE TABLE gossip_performance_impacts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    crew_member_id UUID NOT NULL REFERENCES crew_members(id) ON DELETE CASCADE,
    gossip_id UUID NOT NULL REFERENCES gossip(id) ON DELETE CASCADE,
    
    impact_type VARCHAR(50) NOT NULL, -- 'morale', 'stress', 'productivity'
...
LEFT JOIN gossip_network_stats gns ON cm.id = gns.crew_member_id AND cm.ship_id = gns.ship_id
